...
| 단계 | 기능 | 담당 | 상태 |
|---|---|---|---|
| ① CAD 파싱 | 레이어명 기반 벽체 중심선·창호 블록 추출, 텍스트 파싱 | 황혜선 | ✅ 완료 |
| ② Revit 객체 생성 | 벽체 자동 생성, FamilySymbol 복제·창호 배치 | 황혜선 | ✅ 완료 |
| ③ 제품 DB 조회 | Excel 기반 제품 목록 로드 | 이태권 | 🔧 진행 중 |
| ④ 조건 필터 | 제조사·프레임·치수(W×H) AND 필터, 카드 UI 자동 갱신 | 손수진 | 🔧 진행 중 |
| ⑤ 제품 선택 | 카드뷰 제품 선택 | 손수진 | 🔧 진행 중 |
| ⑥ 파라미터 업데이트 | 선택 제품 → Revit 패밀리 파라미터 반영 | 이태권 | 🔧 진행 중 |
| ⑦ 일람표 동기화 | 창호 스펙·단가 일람표 자동 생성 | 이태권 | ✅ 완료 |
| - | GitHub/dev structure | 손수진 | ✅ 완료 |
| - | UX/UI | 손수진 | 🔧 진행 중 |
| - | Grasshopper 연동 | 안아영 | ✅ 완료 |
...
기능 단위로 브랜치를 분리해 독립적으로 개발하고, PR 리뷰 후 main에 머지합니다.

```
main  (항상 안정적인 상태 유지)
  │
  ├── feature/cad-parse            ← ① CadParser.cs           ✅
  ├── feature/revit-place-family   ← ② FamilyPlacer.cs        ✅
  ├── feature/db-query             ← ③ ProductCatalog.cs      🔧
  ├── feature/filter-condition     ← ④ ProductFilter.cs       🔧
  ├── feature/product-select       ← ⑤ ProductSelector.cs     🔧 현재 작업 중
  ├── feature/param-update         ← ⑥ ParameterUpdater.cs   🔧
  └── feature/schedule-sync        ← ⑦ ScheduleManager.cs    ✅
```

> `main` 직접 push 금지. 반드시 PR → 코드 리뷰 → 머지 순서로 진행.
